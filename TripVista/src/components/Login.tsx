...
function Login() {
    ...
    const isLogin = useAppSelector(s => s.RootReducer.Package.isLogin);
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (!result || result?.data.validationResult.errors.length > 0) {
            alert(result?.data?.validationResult?.errors[0]?.message);
        }
        else {
    ...
    return (
        <>
            <Snackbar
                ...
                onClose={handleClose}
                message="Before continue on package booking, please login."
                key={'top' + 'right'}
            />
